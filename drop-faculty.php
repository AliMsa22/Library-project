...
// Fetch faculties for the dropdown menu
$sql = "SELECT id, name FROM faculties WHERE is_deleted = 0";
...
if (isset($_POST['submit'])) {
    ...
    // Delete the selected faculty from the faculties table
    $deleteSql = "UPDATE faculties SET is_deleted = 1 WHERE id = :facultyId";
    $deleteQuery = $dbh->prepare($deleteSql);
    $deleteQuery->bindParam(':facultyId', $facultyId, PDO::PARAM_INT);
    $deleteQuery->execute();

    // Check if the deletion was successful
    if ($deleteQuery->rowCount() > 0) {
        $msg = "Faculty deleted successfully.";
        // Update faculties for the dropdown menu
        $sql = "SELECT id, name FROM faculties WHERE is_deleted = 0";
        $query = $dbh->prepare($sql);
        $query->execute();
        $faculties = $query->fetchAll(PDO::FETCH_OBJ);
        //Delete majors of faculty deleted
        $delete = "UPDATE majors SET is_deleted = 1 WHERE faculty_id = :facultyId AND is_deleted = 0";
        $deleteMajor = $dbh->prepare($delete);
        $deleteMajor->bindParam(':facultyId', $facultyId, PDO::PARAM_INT);
        $deleteMajor->execute();
        //Delete subjects of faculty deleted
        $lastDelete = "UPDATE subjects SET is_deleted = 1 WHERE major_id IN (SELECT major_id FROM majors WHERE faculty_id = :facultyId AND is_deleted = 1)";
        $deleteSubject = $dbh->prepare($lastDelete);
        $deleteSubject->bindParam(':facultyId', $facultyId, PDO::PARAM_INT);
        ...
        $error = "Error while deleting faculty.";
    }
}
?>
...
<head>
    ...
    <title>Online Library Management System | Drop Faculty</title>
    <!-- Bootstrap core CSS -->
    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="row pad-botm">
                <div class="col-md-12">
                    <h4 class="header-line">Drop Faculty</h4>
                </div>
            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="facultyId" required>
                                        ...
                                        <?php foreach ($faculties as $faculty) { ?>
                                            <option value="<?php echo htmlentities($faculty->id); ?>">
                                                <?php echo htmlentities($faculty->name); ?>
                                            </option>
                                        <?php } ?>
                                    </select>
                                </div>
